...
def generate_launch_description():
    ...
    rc_car_description_path = os.path.join(
        get_package_share_directory('rc_car_description'))
    
    rc_car_bringup_path = os.path.join(
    ...
    gazebo_resource_path = SetEnvironmentVariable(
        ...
        value=[
            os.path.join(rc_car_bringup_path, 'worlds'), ':' +
            str(Path(rc_car_description_path).parent.resolve())
            ]
        )

    arguments = LaunchDescription([
                DeclareLaunchArgument('world', default_value='simple_world',
                          description='Basic Gazebo sim World'),
           ]
    )
    ...
    xacro_file = os.path.join(rc_car_description_path,
                              'urdf',
                              'robot.urdf.xacro')
    ...
    rviz_config_file = os.path.join(rc_car_bringup_path, 'rviz', 'view_bot.rviz')
